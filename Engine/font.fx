...
float4 FontPixelShader(PixelInputType input) : SV_TARGET
{
	...
	// If the color is black on the texture then treat this picel as transparent.
	if (color.r == 0.0f)
	{
		color.a = 0.0f;
	}

	// If the color is other than black on the texture then this is a picel in the font so draw it using the font fixel color.
	else
	{
		color.a = 1.0f;
