...
bool LightShaderClass::InitializeShader(ID3D10Device* device, HWND hwnd, WCHAR* filename)
{
	...
	//HLSL light shader needs a direction and diffuse color vector. We create pointers
	//that point to the lightDirection and diffuseColor var inside the shader.
...
void LightShaderClass::RenderShader(ID3D10Device* device, int indexCount)	//RenderShader calls the shader
